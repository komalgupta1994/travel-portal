...
export class BookingService {
    ...
    public async book(userInfo, bookingDto: BookingDto) {
        ...
        if ((roomData.quantity - bookedRoomCount) >= bookingDto.quantity) {
            ...
            const savedData = await this.bookingRepository.createQueryBuilder()
                .insert()
                .into(Booking)
                .values(booking)
                .execute();
            this.redisService.setKey(`booking:${savedData.identifiers[0].id}`, 'Reserved', keyExpirationTime);
    ...
    private async checkRoomAvailability(bookingDto: BookingDto): Promise<Booking[]> {
        const booking = await this.bookingRepository.createQueryBuilder('booking')
            ...
            .andHaving(`(booking.check_in_dt >= :check_in_date AND booking.check_in_dt < :check_out_date) OR 
    ...
    private async getBookingByUserId(userId: number) {
        return await this.bookingRepository.createQueryBuilder('booking')
            .where('booking.user_id = :userId AND booking.status = :status', { userId, status: 'Reserved' })
            .getOne();
    }
}
